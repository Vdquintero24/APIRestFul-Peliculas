...
const validarApiKey    = require('../middlewares/validateApiKey');
...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(201).json({ mensaje: 'Película creada exitosamente', data: nueva });
  } catch (error) {
    res.status(error.status || 500).json({ error: error.message });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    res.status(200).json({ mensaje: 'Película actualizada exitosamente', data: actualizada });
  } catch (error) {
    res.status(error.status || 500).json({ error: error.message });
  }
});
